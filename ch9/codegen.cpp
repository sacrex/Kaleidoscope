...
Function *PrototypeAST::codegen()
{
	...
	// but for main function: void(double, double).
...
Function *FunctionAST::codegen()
{
	...
	DISubprogram *SP = DBuilder->createFunction(FContext, P.getName(), StringRef(), Unit, 
					LineNo, CreateFunctionType(TheFunction->arg_size(), Unit), ScopeLine,
	...
	// Unset the location for teh prologue emission (leading instructions with no
	...
	for (auto &Arg : TheFunction->args()) {
		...
		DILocalVariable *D = DBuilder->createParameterVariable(SP, Arg.getName(), ++ArgIdx, Unit,
						LineNo, KSDbgInfo.getDoubleTy(), true);

		DBuilder->insertDeclare(Alloca, D, DBuilder->createExpression(),
						DebugLoc::get(LineNo, 0, SP), Builder.GetInsertBlock());
